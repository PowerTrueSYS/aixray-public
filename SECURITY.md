...
re-scans its transformed output for original values, residual
identifier-shaped values, unissued pseudotokens, and evidence outside its
allowlist. If validation or publication fails, it exits nonzero and publishes
no final review HTML. Strict validation can reject a report rather than guess;
contact PowerTrue without an attachment if that occurs. Even after success,
open and inspect the review HTML before sending it.
