#!/bin/ksh
# AIXray outbound pseudonymized review-copy helper.
# Reads one local AIXray HTML report and performs no network action.
set -u

# Must equal VERSION in src/aixray-aix.sh.in and AIXRAY_STANDALONE_VERSION in
...
AIXRAY_REVIEW_PACK_VERSION="1.2.0"
...
Create a separate pseudonymized review candidate from a current AIXray native
...
machine. AIXray sends nothing and performs no network action.
...
VALIDATOR_PROGRAM=$PROGRAM_DIR/aixray-review-validate.awk
...
if [ "$REPORT_MARKER_COUNT" != "1" ]; then
  echo "$PROGRAM: unsupported input: expected AIXray report version 1 exactly once" >&2
  exit 2
fi
...
END {
  ...
  if (DATE_COUNT != 1 \
    ...
    reject("expected one valid AIXray report date marker")
  month = substr(REPORT_DATE, 6, 2) + 0
  day = substr(REPORT_DATE, 9, 2) + 0
  if (month < 1 || month > 12 || day < 1 || day > 31) \
    reject("AIXray report date marker is outside the calendar range")
  if (HOST_COUNT != 1 || REPORT_HOST !~ /^[A-Za-z0-9][A-Za-z0-9_.-]*$/) \
    reject("expected one valid AIXray report host marker")
}
' "$INPUT_PATH")
PREFLIGHT_STATUS=$?
